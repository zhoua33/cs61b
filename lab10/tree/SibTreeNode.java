...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(isValidNode()){
	if(c<1) c = 1;
	SibTreeNode firstch = this.firstChild;
	if(firstch == null){
	...
	if(c == 1){
		...
		this.firstChild = newnode;
		newnode.parent = this;
		firstChild.nextSibling = firstch;
		this.myTree.size++;
		return;
	}
	while(firstch.nextSibling != null && c>2){
		c --;
		firstch = firstch.nextSibling;
	}
	SibTreeNode newnode = new SibTreeNode(this.myTree,item);
	newnode.nextSibling = firstch.nextSibling;       //if this is the tail of the list, newnode.nextSibling = null
	firstch.nextSibling = newnode;
	newnode.parent = this;
	myTree.size++;
	return;
	}else{
  ...
  public void removeLeaf() throws InvalidNodeException {
	...
	if(this.firstChild == null){
		...
		newnode = newnode.nextSibling;
	      ...
	      if(formnode != null){
		formnode.nextSibling = this.nextSibling;
		}else{
